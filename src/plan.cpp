...
std::pair<std::vector<Path>, int> multipathplan(const SlopeAtlas &slopeAtlas, const TerrainMapFloat &commsMap,
  ...
  auto getCost = [&slopeAtlas](const Path::State &a, const Path::State &b) -> double {
    ...
    return config.slopeCostMultiplier * (slope / 90.0) + config.distCostMultiplier * dist +
  ...
  while (true) {
    ...
    for (const auto &succ : succs) {
      ...
      if (nodeMap.find(succ) == nodeMap.end() || tentativeG < nodeMap.at(succ).gscore) {
...
std::pair<std::vector<int>, int64_t> solveTSP(const Eigen::MatrixXd &costs, int depot = 0) {
  ...
  RoutingIndexManager manager(data.distance_matrix.size(), data.num_vehicles, data.depotNode);
  RoutingModel routing(manager);

  const int transit_callback_index =
      routing.RegisterTransitCallback([&data, &manager](int64_t from_index, int64_t to_index) -> int64_t {
