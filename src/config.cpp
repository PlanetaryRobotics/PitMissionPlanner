...
void parseConfigFile(PlannerConfiguration &config, const std::string &fileName) {
  ...
  config.roverLongitudinalSlopeLimit =
      tbl["Rover"]["LongitudinalSlopeLimit"].value_or(config.roverLongitudinalSlopeLimit);
...
void parseCommandLine(PlannerConfiguration &config, int argc, char *argv[]) {
  ...
  if (cmdl[{"-h", "--help", "--Help"}]) {
    ...
    fmt::print(
        "      -nc,     --NumCandidates        The number of vantage candidates to consider for vantage selection.\n");
    fmt::print("      -va,     --MaxVisAngle          The maximum incidence angle (in degrees) that is considered a "
               "valid view hit.\n");
    fmt::print("      -vr,     --MaxVisRange          The maximum distance (in meters) beyond which a view hit is not "
               "counted.\n");
    fmt::print("      -minsep, --MinVantageSeparation The minimum distance between any two vantages.\n");
    fmt::print("\n");

    fmt::print("      -dmult,  --DistCostMultiplier   The unitless multiplier used to weight the distance cost term in "
               "the planning cost function.\n");
    fmt::print("      -smult,  --SlopeCostMultiplier  The unitless multiplier used to weight the slope cost term in "
               "the planning cost function.\n");
    fmt::print("      -tmult,  --TurnCostMultiplier   The unitless multiplier used to weight the turning cost term in "
               "the planning cost function.\n");
    fmt::print("      -hmult,  --HeuristicMultiplier  The unitless multiplier used to inflate the A* heuristic for "
               "faster, less-optimal planning.\n");
    fmt::print("\n");
