...
TerrainMesh::TerrainMesh(const std::string &filename) : filename(filename) {
    ...
    float *vb = (float *)rtcSetNewGeometryBuffer(geometry, RTC_BUFFER_TYPE_VERTEX, 0, RTC_FORMAT_FLOAT3, 3 * sizeof(float),
                                                 vertexPositions.size() / 3);
    std::memcpy(vb, vertexPositions.data(), vertexPositions.size() * sizeof(float));

    unsigned *ib = (unsigned *)rtcSetNewGeometryBuffer(geometry, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT3, 3 * sizeof(unsigned),
                                                       faceIndices.size() / 3);
