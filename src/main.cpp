...
int main(int argc, char *argv[]) {
  ...
  if (landingSiteI < 0 || landingSiteI >= elevationMap.rows || landingSiteJ < 0 || landingSiteJ >= elevationMap.cols) {
    throw std::runtime_error(fmt::format("Landing site at ({}, {}) is outside of map boundaries ({}, {}).",
...
#pragma omp parallel for
    for (int f = 0; f < path.states.size(); ++f) {
      ...
      for (int ff = 0; ff < f; ++ff) {
        ...
        drawLine(img, s0.i * upsample, s0.j * upsample, s1.i * upsample, s1.j * upsample, cm::Color(1.0, 1.0, 1.0),
                 1.0 * upsample);
      }
      ...
      for (int ff = 1; ff < f; ++ff) {
        ...
        if (std::find(allSites.begin(), allSites.end(), s) != allSites.end()) {
          drawTriangle(img, s.i * upsample, s.j * upsample, (int)s.d * 45, cm::Color(1.0, 1.0, 1.0), 5 * upsample,
                       8 * upsample);
        }
      }

      // Draw the rover.
      drawTriangle(img, s.i * upsample, s.j * upsample, (int)s.d * 45, cm::Color(1.0, 0.0, 0.0), 5 * upsample,
                   8 * upsample);
